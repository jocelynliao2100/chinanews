...
def plot_data(year_month_counts, column_names):
...
def parse_list_docx(file_content):
    ...
    items = []
    for li in soup.select("ul.scdList li"):
        span = li.find("span")
        date_str = span.text.strip("[] ") if span else ""
        a = li.find("a")
        title = a["title"] if a and a.has_attr("title") else (a.text if a else "")
...
if uploaded_files_count and len(uploaded_files_count) == 5:
    ...
    if year_month_counts:
        plot_data(year_month_counts, original_column_names)
    else:
